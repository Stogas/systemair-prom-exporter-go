...
func GetFanRPM(client *modbus.ModbusClient, fan string) (uint16, error) {
	switch fan {
	case "SAF":
	...
	return 0, fmt.Errorf("unknown fan %q", fan)
}

// GetFanPercentage gets the "SAF/EAF fan speed" in percentage
// Provide "SAF" or "EAF" as a value to select the wanted fan sensor.
// Min 0 %, Max 100 %.
func GetFanPercentage(client *modbus.ModbusClient, fan string) (uint16, error) {
	switch fan {
	case "SAF":
